...
import { NavigationParams } from 'react-navigation/lib/TypeDefinition';
...
export function paramsEqual(params1: ?NavigationParams, params2: ?NavigationParams) {
  if (params1 === params2) {
    return true;
  }
...
export const reducer = (state, action) => {
  ...
  // Somewhat hacky way of ensuring we don't push the same route twice
  // (quick double tap of button etc.)

  if (includes(NavigationActions, action.type)) {
    const currentRoute = state.routes[state.index];

    // TODO: Is this correct for nested navigators (action != null)?
    if (currentRoute.routeName === action.routeName && paramsEqual(
    ...
    return Immutable(
      RootNavigator.router.getStateForAction(action, state.asMutable({ deep: true })),
    );
  }
